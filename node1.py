...
class AirNode:
    ...
    def become_master(self):
        ...
        for node in air_nodes:
            ...
            while eof_count < 2:
                try:
                    with open("out.txt", "r") as f:
                        text = f.read()
                        eof_count = text.count("EOF_MARKER")
                        print(eof_count)
                except FileNotFoundError:
                    eof_count = 0
                time.sleep(1)

            print("3 EOF_MARKERs found. Terminating BPSK_RX.")
            if self.bpsk_rx_process and self.bpsk_rx_process.poll() is None:
                self.bpsk_rx_process.terminate()
                self.bpsk_rx_process = None
            ...
            time.sleep(3)  # wait 3 seconds
            ack_process.terminate()
            # Step 7: Extract valid data from out.txt
            print("Extracting valid data from out.txt...")
            extract_valid_transmission("out.txt", f"{self.identifier.lower()}_{node.lower()}_data.txt")
            print(f"Saved cleaned transmission to {self.identifier.lower()}_{node.lower()}_data.txt")
    ...
    def become_slave(self):
        ...
        files = [f for f in os.listdSlaveir() if f.startswith("collected_data_") and f.endswith(".txt")]
